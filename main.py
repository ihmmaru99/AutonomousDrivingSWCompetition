...
def make_message(errorFromMid, speed):
    send2arduino = ''
    send2arduino += 'S'
    if errorFromMid < 0:
        send2arduino += '-'
        ...
        send2arduino += '+'
    errorFromMid = str(abs(errorFromMid))
    errorFromMid = errorFromMid.zfill(2)
    send2arduino += errorFromMid
    speed = str(speed)
    speed = speed.zfill(3)
    send2arduino += speed
    return send2arduino

def pidControl(errorFromMid):

    global P_GAIN, I_GAIN, D_GAIN
    global pError, iError, dError
    
    castError = errorFromMid
    dError = castError - pError
    pError = castError
    iError += castError
    if(abs(castError) < 10):
        mError = 0
    return P_GAIN * pError + I_GAIN * iError + D_GAIN * dError
